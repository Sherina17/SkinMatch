...
def read_products():
    ...
    products["search_text"] = (
        products["skin_type_clean"]
        + " "
        + products["concerns_clean"]
        + " "
...
def get_recommendations(skin_type, concerns=None):
    ...
    for index, row in df.iterrows():
        ...
        if skin_match and concern_match:
...
def images(filename):
    ...
    if image_dir.exists():
        print("[DEBUG] FILES IN IMAGE FOLDER:")
        for f in image_dir.iterdir():
            print("[DEBUG]", repr(f.name))
